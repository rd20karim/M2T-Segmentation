...
class Encoder(nn.Module):
    def __init__(self, input_dim,hidden_size, embedding_dim, num_layers=1, bidirectional=True,type='pose',
        ...
        dims = embedding_dim if not self.type == 'pose' else 44 if joint_angles else n_joints*3
    ...
    def forward(self, x, previous_hidden,src_lens=None,norm=False):
...
class Decoder(nn.Module):
    def __init__(self, input_dim, hidden_size, embedding_dim, batch_size=32, attention="bahadanau", enc_bidirectional=True,
        ...
        #in_features= embedding_dim + hidden_size + k * num_layers * self.dec_hidden_size
    ...
    def calculate_attention_weight(self, previous_hidden_dec,enc_masks=None,enc_outputs=-1):
        ...
        if "local" in self.attention_type:
            # dot-product (general) # softmax(tanh(h_j.T*W.s_i))-->att_scores
    ...
    def forward(self, x, previous_hidden, encoder_outputs,enc_masks=None,epoch=0,epsilon=0):
        ...
        if self.attention_type:
           ...
           # encoder_outputs, unpacked_len = torch.nn.utils.rnn.pad_packed_sequence(encoder_outputs)
            src_len = encoder_outputs.size(0)
            ...
            if "local" in self.attention_type:
                ...
                f_ht = torch.sigmoid( self.scale*self.proj_vector(self.position_layer(previous_hidden)))
...
class seq2seq(nn.Module):
    def __init__(self, input_dim, hidden_size, embedding_dim, num_layers=1,device=torch.device('cpu'),dropout=0,beam_size=1,
        ...
        self.enc_pose = Encoder(input_dim, hidden_size, embedding_dim, num_layers=num_layers, dropout=dropout,
                                joint_angles=joint_angles, bidirectional=bidirectional,encoder_type=encoder_type,hidden_dim=hidden_dim).to(self.device)
        ...
        self.dec = Decoder(input_dim, hidden_size, embedding_dim, num_layers=num_layers, device=device,
                           ...
                           beam_search=True if self.beam_size>1 else False,D=D,scale=scale).to(self.device)
        self.attention_type = attention
        self.num_layers = num_layers


    def forward(self, x, y, init_hidden, teacher_force_ratio=0,src_lens=None):
        ...
        if "GRU" in self.encoder_type:
            ...
            #if type =='pose' else self.enc_lang(x, init_hidden,src_lens=self.src_lens)
        # todo special for MLP the line bellow
        else:
        ...
        trg_len = y.size(0)  # src_len==trg_len case of the autoencoder
        output_list = []
        # Initial hidden state if decoder calculate from the backward and forward

        self.attention_weights_list = []  # torch.emself.pty((src_len,batch_size,trg_len))
        ...
        # Set a good initialization comment
        # v_mask = torch.norm(x[:, :, 1:] - x[:, :, :-1], 2, dim=-1) > 0.045
        # self.dec.pt  = torch.argmax(v_mask.int(), dim=0)
