...
class vocabulary():
    ...
    def token_correction(self,ask_user=False):
        ...
        # with open("../datasets/corrections.txt") as f:
        #     corrections = f.read()
        # replace = json.loads(corrections)
        ...
        for idx, desc in enumerate(self.sentences):
            ...
            tokens = self.language_tokenizer(desc)
                ...
                for token in list(misppelled):
                    ...
                    if not ask_user or not user_correction: # ask_user is first evaluated
                        try:
                            ...
                            #tokens[tokens.index(token)] = spell.correction(token) if spell.correction(token) not in {'i',None} else token
                            logging.info("Token index %d corrected %s ----> %s " % (idx, token, tokens[tokens.index(token)]))
                        except ValueError:
            ...
            # FOR USER EXAMINATION:
            try:
                with open("sentences_corrections"+".csv", mode="a",encoding='utf-8') as fa:
                    fa.write("%s \t %s\n" % (self.old_sentences[idx].replace("\n","").replace("\r",""),self.corrected_sentences[idx-shift]))
            except IndexError:
    ...
    def build_vocabulary(self, min_freq=1):
        ...
        with open("./dataset_vocab_1.txt", mode='w',encoding='utf-8') as fw:
